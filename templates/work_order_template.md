...
**Start Command (for manual development):**
```bash
# From within the target repository directory (hub/ or evv/)
bash scripts/start-agent-env.sh [WORK_ORDER_ID]
```

**Access URL:** Dynamic (the script will print the URL)
...
The `run-tests.sh` script AUTOMATICALLY performs these checks. If the script exits with code 0, you have met these requirements.

- [x] **`tests/__init__.py` is correct:** The script will fail if Odoo doesn't discover your tests.
- [x] **Your module appears in test output:** The script verifies this.
- [x] **Test count is correct:** The script verifies this.

**If the script fails:**
- ❌ Your tests didn't run or failed.
- Fix the issue (likely `tests/__init__.py` or a failing assertion).
- Re-run the script.
- DO NOT proceed until the script passes.

**Provide:** The `proof_of_execution_tests.log` file generated by the script.

### 9.2 Boot Verification (If Required by Work Order)
```bash
# Start a persistent environment for boot/upgrade tests
bash scripts/start-agent-env.sh [WORK_ORDER_ID]
# The script will print the PROJECT_NAME (e.g., evv-agent-wo-xxx)

# Wait, then capture logs
sleep 30
docker-compose -p [PROJECT_NAME] logs --tail="100" odoo
```
**Provide:** Last 50-100 lines of boot log, confirming a clean start.

### 9.3 Module Upgrade Test (If Required by Work Order)
```bash
# Use the persistent environment from the boot test
docker-compose -p [PROJECT_NAME] exec odoo odoo-bin -c /etc/odoo/odoo.conf -d postgres -u [module_name] --stop-after-init
```
